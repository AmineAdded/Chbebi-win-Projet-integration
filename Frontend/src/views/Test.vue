<template>
    <div>
      ...
      <v-container v-else fluid class="quiz-container">
        <v-sheet class="quiz-wrapper rounded-lg" color="transparent">
          ...
          <v-sheet class="progress-tabs mb-6 mx-auto rounded-pill" max-width="700px">
            <v-row no-gutters>
              <v-col v-for="(tab, index) in 5" :key="index">
                <v-sheet 
                  class="pa-2 text-center rounded-pill mx-1" 
                  :color="currentTab === index ? '#1e2a38' : 'lightblue'" 
                  height="40"
          ...
          <!-- Question Card -->
          <v-card 
            class="question-card mx-auto mb-8" 
            max-width="700px" 
            elevation="3"
            rounded="xl"
          >
            ...
            <v-card-text class="answer-options pt-4">
              <v-sheet 
                ...
                :key="index"
                class="answer-option mb-3 pa-3 d-flex align-center rounded-lg"
                elevation="1"
  ...
  export default {
    ...
    methods: {
      ...
      nextQuestion() {
        // Reset selection and move to next tab/question
